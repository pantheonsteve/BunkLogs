...
from .serializers import CounselorLogSerializer
from .serializers import ItemCategorySerializer
from .serializers import ItemSerializer
from .serializers import OrderCreateSerializer
from .serializers import OrderSerializer
from .serializers import OrderTypeSerializer
from .serializers import SimpleItemSerializer
from .serializers import UnitSerializer
from .serializers import UnitStaffAssignmentSerializer
...
class BunkLogsInfoByDateViewSet(APIView):
    ...
    def get(self, request, bunk_id, date):
        ...
        try:
            ...
            logger.exception(f"Error in BunkLogsInfoByDateViewSet for bunk {bunk_id}, date {date}, user {user.email}: {e!s}")
            return Response({"error": f"Internal server error: {e!s}"}, status=500)
...
class UnitStaffAssignmentViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        try:
            ...
            msg = "UnitStaffAssignment matching query does not exist."
            raise Http404(msg)
...
class CamperViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can create campers."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can update campers."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can delete campers."
            raise PermissionDenied(msg)
        instance.delete()
...
class CamperBunkAssignmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can create camper bunk assignments."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can update camper bunk assignments."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can delete camper bunk assignments."
            raise PermissionDenied(msg)
        instance.delete()
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.role == "Counselor":
            ...
            if not has_assignment:
                msg = "You are not authorized to create logs for this bunk."
                raise PermissionDenied(msg)
    ...
    def perform_update(self, serializer):
        ...
        if user.role == "Unit Head":
            ...
            if has_access:
                serializer.save()
                return
            msg = "You are not authorized to update logs for this bunk."
            raise PermissionDenied(msg)
        ...
        if user.role == "Counselor":
            ...
            if instance.counselor.id != user.id:
                msg = "You can only update logs you created."
                raise PermissionDenied(msg)
            ...
            if today != log_created_date:
                msg = "You can only update logs on the day they were created."
                raise PermissionDenied(msg)
        ...
        # Default: deny access
        msg = "You are not authorized to update this bunk log."
        raise PermissionDenied(msg)
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if not (user.is_staff or user.role in ["Admin", "Camper Care"]):
            ...
            if order.user != user:
                msg = "You can only update your own orders."
                raise PermissionDenied(msg)
            if order.order_status != "submitted":
                msg = "You can only update orders that are in 'Submitted' status."
                raise PermissionDenied(msg)
        ...
        if "order_status" in serializer.validated_data:
            ...
            if order.order_status == "submitted" and new_status != "submitted":
                ...
                if not (user.is_staff or user.role in ["Admin", "Camper Care"]):
                    msg = "Only Camper Care and Admin users can update order status from 'Submitted'."
                    raise PermissionDenied(msg)
    ...
    def perform_destroy(self, instance):
        ...
        if not (user.is_staff or user.role in ["Admin", "Camper Care"]):
            ...
            if instance.user != user:
                msg = "You can only delete your own orders."
                raise PermissionDenied(msg)
            if instance.order_status != "submitted":
                msg = "You can only delete orders that are in 'Submitted' status."
                raise PermissionDenied(msg)
...
class ItemViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can create items."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can update items."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can delete items."
            raise PermissionDenied(msg)
        instance.delete()
...
class ItemCategoryViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can create categories."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can update categories."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can delete categories."
            raise PermissionDenied(msg)
        instance.delete()
...
class OrderTypeViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can create order types."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can update order types."
            raise PermissionDenied(msg)
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if not self.request.user.is_staff:
            msg = "Only staff members can delete order types."
            raise PermissionDenied(msg)
        instance.delete()
