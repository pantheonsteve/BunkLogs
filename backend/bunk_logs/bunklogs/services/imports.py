...
def _validate_score(score: str | None) -> int | None:
    ...
    try:
        ...
        msg = f"Invalid score format: {score}"
        raise BunkLogImportError(msg)

def import_bunk_logs_from_csv(file_path: str | Path, *, dry_run: bool = False, default_counselor_email: str | None = None) -> dict:
    """
    ...
    if default_counselor_email:
        try:
            ...
            msg = f"Default counselor with email {default_counselor_email} not found"
            raise BunkLogImportError(msg)
    ...
    with open(file_path, encoding="utf-8") as csvfile:
        ...
        for field in required_fields:
            if field not in reader.fieldnames:
                msg = f"CSV file is missing required field: {field}"
                raise BunkLogImportError(msg)
        ...
        for i, row in enumerate(reader):
            try:
                ...
                if not counselor_email and not default_counselor:
                    msg = "Counselor email is required when no default counselor is provided"
                    raise BunkLogImportError(msg)
                ...
                try:
                    if " - " in bunk_full_name:
                        ...
                        msg = "Bunk name must be in format 'cabin_name - session_name'"
                        raise ValueError(msg)
                except ValueError:
                    msg = f"Invalid bunk name format: {bunk_full_name}. Expected format: 'cabin_name - session_name'"
                    raise BunkLogImportError(msg)
                ...
                try:
                    ...
                    try:
                        ...
                        if not bunk_obj.is_active:
                            msg = f"Bunk '{bunk_full_name}' exists but is not active"
                            raise BunkLogImportError(msg)
                    except Bunk.DoesNotExist:
                        msg = f"Bunk with cabin '{cabin_name}' and session '{session_name}' does not exist"
                        raise BunkLogImportError(msg)
                except Bunk.MultipleObjectsReturned:
                    msg = f"Multiple active bunks found with cabin '{cabin_name}' and session '{session_name}'"
                    raise BunkLogImportError(msg)

                # Find the counselor
                try:
                    counselor = User.objects.get(email=counselor_email) if counselor_email else default_counselor
                except User.DoesNotExist:
                    msg = f"Counselor with email '{counselor_email}' does not exist"
                    raise BunkLogImportError(msg)
                ...
                try:
                    ...
                    msg = f"No active assignment found for: {camper_first_name} {camper_last_name} in {bunk_full_name}"
                    raise BunkLogImportError(
                        msg,
                    )
                except CamperBunkAssignment.MultipleObjectsReturned:
                    msg = f"Multiple active assignments found for: {camper_first_name} {camper_last_name} in {bunk_full_name}"
                    raise BunkLogImportError(
                        msg,
                    )
