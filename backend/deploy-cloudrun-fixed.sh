...
gcloud run deploy $SERVICE_NAME \
  ...
  --update-env-vars USE_CLOUD_SQL_AUTH_PROXY=True \
  --update-env-vars GS_BUCKET_NAME=bunk-logs-static \
  --update-env-vars DJANGO_SETTINGS_MODULE=config.settings.production \
  --update-env-vars "DATABASE_URL=postgresql://stevebresnick:April221979!@bunk-logs-clc?host=/cloudsql/bunklogsauth:us-central1:bunk-logs" \
  --update-env-vars "SECRET_KEY=\$#u&&du@pa3k)bn-)7s&hvr=i#a*qs9t=@g!%y5huhw)g9&yi!" \
  --update-env-vars "ALLOWED_HOSTS=bunklogs.net,*.run.app,localhost:5173" \
  --update-env-vars POSTGRES_USER=stevebresnick \
  --update-env-vars "POSTGRES_PASSWORD=April221979!" \
  --update-env-vars POSTGRES_HOST=/cloudsql/bunklogsauth:us-central1:bunk-logs \
  --update-env-vars POSTGRES_PORT=5432 \
  --update-env-vars POSTGRES_DB=bunk-logs-clc \
  --max-instances=10 \
  --min-instances=0 \
  --memory=1Gi \
  --cpu=1 \
