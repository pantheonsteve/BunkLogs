...
1. Backend: new models in `core` app (or a new `notes` app — see Implementation note below):
   1. `Note` — fields: `id`, `organization` (FK, OrgScoped), `program` (FK), `author` (FK Person), `author_role_at_write` (CharField — captures the Membership role the author held at submission, for audit and display), `subject` (CharField max 200), `body` (TextField max 10000), `camper_reference` (FK Person nullable), `source_content_type` (CharField nullable — one of `'reflection_concern'`, `'specialist_note'`), `source_object_id` (CharField max 50 nullable — holds the PK of the source object as a string; `Reflection.id` is an integer AutoField so this must be `CharField` rather than `UUIDField` to accommodate both integer PKs and any future UUID-keyed sources), `created_at`, `archived_by` (M2M to Person through `NoteArchive`).
...
6. Backend: cross-reference handlers.
   ...
   2. `POST /api/v1/notes/from-specialist-note/` — Story 70 entry. Body: `reflection_id`. There is no separate `SpecialistNote` model; a "specialist note" is a `Reflection` with `template.role='specialist'` and `subject_mode='single_subject'`. Validates: the referenced `Reflection` has a specialist-role template (returns 400 otherwise); requester is a Counselor with active Membership on the camper's bunk OR the Reflection's author. Rejects if the reflection is marked sensitive (per Story 70 criterion 9). Returns a draft note payload pre-filled per Story 70 criterion 4.
...
13. Tests:
    ...
    9. Frontend: Vitest integration test (with mocked API) covering the Story 69 compose-and-receive flow: UH reads a concern, the `from-bunk-concern/` mock returns a pre-filled draft, the NoteComposer is opened and submitted, and the counselor's inbox mock confirms the note appears. A true end-to-end browser test (e.g. Playwright) would require a separate `chore/playwright-setup` PR first; do not block 7_19 on that. The Vitest integration test is sufficient for this step.
