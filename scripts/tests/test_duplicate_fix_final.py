#!/usr/bin/env python
"""
Test script to verify the duplicate b    # Create test unit
    unit = Unit.objects.create(name="Test Unit")k log fix.
...
def cleanup_test_data():
    ...
    CounselorBunkAssignment.objects.filter(bunk__unit__name="Test Unit").delete()
...
def create_test_data():
    ...
    # Create a test cabin with required capacity field
    cabin = Cabin.objects.create(
        name="Test Cabin",
        capacity=20  # Required field
    )
    ...
    # Create a counselor bunk assignment
    from bunk_logs.bunks.models import CounselorBunkAssignment
    from datetime import date as date_class
    today = date_class.today()
    counselor_assignment = CounselorBunkAssignment.objects.create(
        counselor=counselor,
        bunk=bunk,
        start_date=today,
        is_primary=True
    ...
    print(f"  Cabin: {cabin.name} (ID: {cabin.id}, capacity: {cabin.capacity})")
    ...
    print(f"  Counselor Assignment: ID {counselor_assignment.id} (Primary: {counselor_assignment.is_primary})")
...
def test_duplicate_creation():
    ...
    bunk_log_data = {
        ...
        'date': today.strftime('%Y-%m-%d')  # Provide the date explicitly
    }
