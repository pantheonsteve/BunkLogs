...
const UserInfo = () => {
    ...
    useEffect(() => {
      const fetchUserData = async () => {
        if (email) {
          ...
          try {
            const response = await axios.get(`https://admin.bunklogs.net/api/v1/users/email/${email}`);
    ...
    useEffect(() => {
      const fetchSpecificUserData = async () => {
        if (isAuthenticated) {
          try {
            const response = await axios.get(`http://admin.bunklogs.net/api/v1/users/email/${email}`);
          } catch (err) {
