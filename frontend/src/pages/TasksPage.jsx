    ...
    >
      ...
      <div className="mt-2 flex gap-2">
        {subject.covered_by_me && onView && (
          <button
            onClick={() => { onView(subject); onClose(); }}
...
function SubjectPill({ subject, onTap }) {
  ...
  function statusIcon() {
    ...
    return <span className="text-gray-400 dark:text-gray-500"><IconCircle /></span>;
  }
...
function SelfSection({ task, onNavigate }) {
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); if (!done) onNavigate(task); } }}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 mt-0.5">
          {done ? <IconCheck /> : <span className="text-gray-400"><IconCircle /></span>}
        </span>
      </div>
...
function SubjectSection({ task, onPillTap }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
      ...
      {/* TODO: prompt 3.21 — multi_subject batch submission is deferred */}
      <div
          ...
          <>
            ...
            <div className="space-y-4" data-testid="tasks-list">
              {tasks.map((task) => {
                ...
                // single_subject and multi_subject both rendered as pill grid
                return (
                  <SubjectSection
                    key={task.id}
                    task={task}
